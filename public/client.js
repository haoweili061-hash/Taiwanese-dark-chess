...
function updateTimerText() {
  ...
  const elapsed = Math.floor((end - game.startedAt) / 1000);
  timerText.textContent = `對局時間 ${formatDuration(elapsed)}`;
}
...
function canCurrentUserAct(ready) {
  ...
  return playerRole === "player" && playerSide === game.currentTurn;
}
...
function renderBoard(ready = true) {
  ...
  if (mode === "online") {
    roomLabel.textContent = `房號 ${roomCode}｜你是${roleName()}｜觀戰 ${spectatorCount} 人${ready ? "" : "｜等待對手"}`;
  }
  ...
  if (!ready) {
    ...
    statusText.textContent = game.comboMode
      ? `第 ${game.turnCount} 回合，${turnName()}連吃中${playerSide === game.currentTurn && playerRole === "player" ? "，輪到你" : "，等待對手"}`
      : `第 ${game.turnCount} 回合，輪到${turnName()}${playerSide === game.currentTurn && playerRole === "player" ? "，輪到你" : "，等待對手"}`;
  } else {
    statusText.textContent = game.comboMode
...
copyRoomBtn.addEventListener("click", async () => {
  try {
    ...
    roomLabel.textContent = `房號 ${roomCode} 已複製｜你是${roleName()}｜觀戰 ${spectatorCount} 人`;
  } catch {
