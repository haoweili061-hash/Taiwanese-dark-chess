...
function publicRoom(room, playerId = null) {
  ...
  const isSpectator = Boolean(playerId && room.spectators && room.spectators[playerId]);
  return {
    roomCode: room.code,
    playerId,
    side,
    ...
    spectatorCount: room.spectators ? Object.keys(room.spectators).length : 0,
...
function applyAction(room, playerId, action) {
  ...
  if (!side && room.spectators && room.spectators[playerId]) return { ok: false, message: "觀戰者不能操作棋局。" };
  ...
  try {
    ...
    if (req.method === "POST" && req.url === "/api/join-room") {
      ...
      if (!existingPlayer && !existingSpectator) {
        if (Object.keys(room.players).length < 2) room.players[playerId] = "black";
        else room.spectators[playerId] = true;
      }
